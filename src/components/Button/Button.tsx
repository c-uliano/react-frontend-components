...
const Button = (props: ButtonProps) => {
    ...
    const renderButton = () => {
        ...
        switch (props.btnType) {
            ...
            case 'Link':
                return <Link onClick={props.onClick} to={props.url} className="btn">{props.text}</Link>;
            case 'input':
                // //TODO: type will need updating for prop
                return <input onClick={props.onClick} type={props.type} value={props.text} className="btn" />;
            case 'anchor':
                return <a onClick={props.onClick} href={props.url} className="btn" target={props.newTab ? "_blank" : undefined}>{props.text}</a>;
            default:
                return <button onClick={props.onClick} type="button" className="btn">{props.text}</button>
        }
    }
